...
from onecv.cv.core import is_channel_first
from onecv.cv.core import to_channel_last
from onecv.cv.io.format import VideoFormat
from onecv.file import create_dirs
from onecv.type import Arrays
from onecv.type import Int2Or3T
from onecv.type import to_4d_array
from onecv.type import to_size
...
class VideoWriter:
	...
	def init_video_writer(self, dst: str):
		...
		self.video_writer = cv2.VideoWriter(
			self.dst, fourcc, self.frame_rate, self.video_size[::-1]  # Must be in [W, H]
		)

		if self.video_writer is None:
			raise FileNotFoundError(f"Cannot create video at: {self.dst}.")
