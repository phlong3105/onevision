...
def draw_box(
    ...
    from onecv.cv.core.normalize import denormalize_naive
...
def draw_rectangle(
    ...
    if batch != batch_rect:
        raise ValueError(f"`image` and `rectangle` must have the same batch size."
                         f" But got: {batch} != {batch_rect}.")
    if num_points != 4:
        raise ValueError(f"`num_points` must be 4. But got: {num_points}.")
...
def draw_semantic_mask(
    ...
    if unlabeled is None:
        raise ValueError(f"`class_labels` must have `unlabeled` key.")
    if bg_color is None:
        raise ValueError(f"Do not support `{encoding}`.")
