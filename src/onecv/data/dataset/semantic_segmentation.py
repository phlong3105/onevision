...
"""Template for (Supervised) Semantic Segmentation datasets.
"""
...
from onecv.cv import get_image_hw
from onecv.cv import is_image_file
from onecv.cv import read_image
from onecv.cv import resize
from onecv.cv import to_tensor
from onecv.cv import VISION_BACKEND
from onecv.cv import VisionBackend
from onecv.data.augment import BaseAugment
from onecv.data.data_class import ClassLabels
from onecv.data.data_class import ImageInfo
from onecv.data.data_class import VisionData
from onecv.data.handler import VisionDataHandler
from onecv.factory import AUGMENTS
from onecv.file import create_dirs
from onecv.file import get_hash
from onecv.type import Augment_
from onecv.type import Int3T
from onecv.utils import console
from onecv.utils import download_bar
from onecv.utils import progress_bar
...
# MARK: - SemanticSegmentationDataset

class SemanticSegmentationDataset(VisionDataset, metaclass=ABCMeta):
    """A base class for all (supervised) semantic segmentation datasets.

    Attributes:
        root (str):
            Root directory that contains: train/val/test/... subdirectories.
        split (str):
            Split to use. One of: ["train", "val", "test"].
        image_paths (list):
            List of all image files.
        semantic_paths (list):
            A list of all semantic segmentation image files.
        label_paths (list):
            A list of all label files that can provide extra info about the
        ...
        class_labels (ClassLabels, optional):
            ...
            dataset.
        shape (Int3T):
            Image shape as [H, W, C] Use to resize the image.
        encoding (str):
    ...
    def __getitem__(self, index: int) -> Any:
        ...
        if self.augment is not None:
            input, target = self.augment.forward(input=input, target=target)
        else:
    ...
    def load_data(self):
        """Load and cache images, semantic images, and labels."""
        # NOTE: Cache labels
        """
        path              = self.semantic_paths[0]
        ...
        if os.path.isfile(cached_label_path):
            ...
            hash  = get_hash(self.label_paths + self.custom_label_paths
                             + self.image_paths + self.semantic_paths)
        ...
        with progress_bar() as pbar:
            for i in pbar.track(
                ...
                label = self.load_label(
                    ...
                    semantic_path     = self.semantic_paths[i],
    ...
    def cache_semantic_images(self):
        """Cache semantic segmentation image into memory for faster training
        (WARNING: large datasets may exceed system RAM).
        """
        for semantic_path in self.semantic_paths:
            if not is_image_file(path=semantic_path):
                return
        ...
        with download_bar() as pbar:
            # Should be max 10k images
            for i in pbar.track(
                range(len(self.semantic_paths)),
                description="Caching semantic segmentation images"
            ):
                # image, hw_original, hw_resized
                (self.data[i].semantic,
                 self.data[i].semantic_info) = self.load_semantic_image(index=i)
                gb       += self.data[i].semantic.nbytes
                # pbar.desc = "Caching semantic segmentation image (%.1fGB)" % (gb / 1E9)
    
    def load_semantic_image(self, index: int) -> tuple[np.ndarray, ImageInfo]:
        """Load 1 semantic segmentation image from dataset and preprocess image.
        ...
        Returns:
            ...
            info (ImageInfo):
                `ImageInfo` object.
        """
        image = self.data[index].semantic
        info  = self.data[index].semantic_info
        
        if image is None:  # Not cached
            path  = self.semantic_paths[index]
            image = read_image(path, backend=self.vision_backend)  # RGB
            if image is None:
                raise ValueError(f"No semantic segmentation image found at: {path}.")
    ...
    def load_mosaic(self, index: int) -> tuple[np.ndarray, np.ndarray]:
        """Load 4 images and target segmentation images and create a mosaic.
        ...
        # NOTE: Create mosaic input and enhancement
        for i, index in enumerate(indices):
            # Load input
            input, info = self.load_image(index=index)
            target, _   = self.load_semantic_image(index=index)
            h, w, _     = info.shape
        ...
        # NOTE: Augment
        """
        input4, target4 = paired_images_random_perspective(
            image1= input4,
            image2= target4,
    ...
    def post_load_data(self):
        ...
        """
        # NOTE: Write semantic segmentation images
        self.write_semantic_images()
    ...
    def write_custom_labels(self):
        ...
        """
        parents     = [str(Path(file).parent) for file in self.semantic_paths]
        stems       = [str(Path(file).stem)   for file in self.semantic_paths]
        stems       = [stem.replace(f"_{self.encoding}", "") for stem in stems]
        ...
        # NOTE: Scan all images and semantic images to get information
        with progress_bar() as pbar:
            for i in pbar.track(
            ...
            for i in pbar.track(
                range(len(self.data)),
                description="[red]Scanning semantic segmentation images"
            ):
                # image, hw_original, hw_resized
                _, self.data[i].semantic_info = self.load_semantic_image(index=i)
    ...
    def write_semantic_images(self):
        """Write semantic segmentation images."""
        with progress_bar() as pbar:
            for i in pbar.track(
                range(len(self.semantic_paths)),
                description="[red]Writing semantic segmentation images"
            ):
                data          = self.data[i]
                semantic_path = self.semantic_paths[i]
                # Force re-write or semantic segmentation image is not found.
                if not is_image_file(path=semantic_path) or self.write_labels:
                    semantic_image = draw_semantic_mask(
                        data         = data,
