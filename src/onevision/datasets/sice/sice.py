...
class SICE(ImageEnhancementDataset):
    ...
    Args:
        exposure (float, list, optional):
            Exposure level to used. Can also be a list.
                - SICE dataset:
                    [`1`, `2`, `3`, `4`, `5`, `6`, `5`] or `*`:
                    Exposure level where `1` is the most under-exposure
                    and `7` is the most over-exposure. When `all`, `*`, or
                    `None`, all alphas will be included. Default: `1`.
    """
    ...
    def __init__(
        ...
        root            : str,
        split           : str                        = "train",
        exposure        : Optional[Union[int, list]] = None,
    ...
    @exposure.setter
    def exposure(self, exposure: Optional[Union[int, list]]):
        exposure = exposure if isinstance(exposure, list) else [exposure]
        if exposure is None or "all" in exposure or "*" in exposure:
    ...
    def list_sice_files(self):
        ...
        with progress_bar() as pbar:
            eimage_pattern = os.path.join(
                self.root, "supervised", self.split, "high", "*.jpg"
            )
            for eimage_path in pbar.track(
                ...
                for exposure in self.exposure:
                    image_path        = os.path.join(image_dir, f"{exposure}.jpg")
...
class SICEDataModule(DataModule):
    ...
    def __init__(
        self,
        dataset_dir: str = os.path.join(datasets_dir, "sice"),
        name       : str = "sice",
        *args, **kwargs
    ):
    ...
    def setup(self, phase: Optional[Phase] = None):
        ...
        """
        console.log(f"Setup [red]SICE[/red] datasets.")
