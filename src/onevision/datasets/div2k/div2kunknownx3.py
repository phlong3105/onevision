...
"""Div2K Unknown X3 dataset and datamodule.
"""
...
@DATASETS.register(name="div2kunknownx3")
class Div2KUnknownX3(ImageEnhancementDataset):
    """Div2K Unknown X3 dataset consists of 800 image pairs for super-resolution
    task.
    """
...
# MARK: - Div2KUnknownX3DataModule

@DATAMODULES.register(name="div2kunknownx3")
class Div2KUnknownX3DataModule(DataModule):
    """Div2K Unknown X3 DataModule."""
    ...
    def setup(self, phase: Optional[Phase] = None):
        ...
        """
        console.log(f"Setup [red]Div2K Unknown X3[/red] datasets.")
        
        # NOTE: Assign train/val datasets for use in dataloaders
        if phase in [None, Phase.TRAINING]:
            self.train = Div2KUnknownX3(
                root=self.dataset_dir, split="train", **self.dataset_kwargs
            )
            self.val = Div2KUnknownX3(
...
if __name__ == "__main__":
    # NOTE: Get DataModule
    cfgs = data
    dm   = Div2KUnknownX3DataModule(**cfgs)
    dm.setup()
