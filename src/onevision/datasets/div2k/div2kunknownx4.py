...
"""Div2K Unknown X4 dataset and datamodule.
"""
...
class Div2KUnknownX4(ImageEnhancementDataset):
    """Div2K Unknown X4 dataset consists of 800 image pairs for super-resolution
    task.
    """
...
class Div2KUnknownX4DataModule(DataModule):
    ...
    def setup(self, phase: Optional[Phase] = None):
        ...
        """
        console.log(f"Setup [red]Div2K Unknown X4[/red] datasets.")
        
        # NOTE: Assign train/val datasets for use in dataloaders
        if phase in [None, Phase.TRAINING]:
            self.train = Div2KUnknownX4(
                root=self.dataset_dir, split="train", **self.dataset_kwargs
            )
            self.val = Div2KUnknownX4(
...
if __name__ == "__main__":
    ...
    dm   = Div2KUnknownX4DataModule(**cfgs)
    dm.setup()
