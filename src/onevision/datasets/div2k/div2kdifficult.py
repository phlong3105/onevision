...
"""Div2K Difficult dataset and datamodule
"""
...
class Div2KDifficult(ImageEnhancementDataset):
    """Div2K Difficult dataset consists of 800 image pairs for super-resolution
    task.
    """
...
class Div2KDifficultDataModule(DataModule):
    ...
    def setup(self, phase: Optional[Phase] = None):
        ...
        """
        console.log(f"Setup [red]Div2K Difficult[/red] datasets.")
        
        # NOTE: Assign train/val datasets for use in dataloaders
        if phase in [None, Phase.TRAINING]:
            self.train = Div2KDifficult(
                root=self.dataset_dir, split="train", **self.dataset_kwargs
            )
            self.val = Div2KDifficult(
...
if __name__ == "__main__":
    ...
    dm   = Div2KDifficultDataModule(**cfgs)
    dm.setup()
