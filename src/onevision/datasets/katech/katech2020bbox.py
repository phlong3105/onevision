...
"""Katech2020 detection dataset and datamodule.
"""
...
from onevision.data import PascalLabelHandler
...
@DATASETS.register(name="katech2020bbox")
class KATECH2020Bbox(ObjectDetectionDataset):
    """KATECH2020 Bbox Dataset.
    """
    ...
    def list_files(self):
        ...
        if self.split not in ["train", "val"]:
            raise ValueError(f"KATECH2020 BBox dataset only supports `split`: "
                             f"`train` or `val`. Get: {self.split}.")
        
        # NOTE: List label & image files
        with progress_bar() as pbar:
            label_pattern = os.path.join(
                self.root, f"katech2020_bbox", self.split, "*",
                "*_annotations_v001_1", "*.xml"
            )
            label_paths = [
                p for p in
                pbar.track(
    ...
    def load_class_labels(self):
        """Load ClassLabels."""
        path = os.path.join(self.root, "katech2020_bbox", "26_class_labels.json")
        if not os.path.isfile(path):
            current_dir = os.path.dirname(os.path.abspath(__file__))
            path = os.path.join(current_dir, "katech2020_26_class_labels.json")
        self.class_labels = ClassLabels.create_from_file(label_path=path)


# MARK: - KATECH2020BboxDataModule

@DATAMODULES.register(name="katech2020bbox")
class KATECH2020BboxDataModule(DataModule):
    """KATECH2020 Bbox DataModule."""
    
    # MARK: Magic Functions
    
    def __init__(
        self,
        dataset_dir: str = os.path.join(datasets_dir, "katech"),
        name       : str = "katech2020bbox",
        *args, **kwargs
    ):
    ...
    def setup(self, phase: Optional[Phase] = None):
        ...
        Args:
            phase (Phase, optional):
                ...
                Default: `None`.
        """
        console.log(f"Setup [red]KATECH2020 BBox[/red] datasets.")
        
        # NOTE: Assign train/val datasets for use in dataloaders
        if phase in [None, Phase.TRAINING]:
            full_dataset = KATECH2020Bbox(
...
if __name__ == "__main__":
    # NOTE: Get DataModule
    cfgs = data
    dm   = KATECH2020BboxDataModule(**cfgs)
    dm.setup()
