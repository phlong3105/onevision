...
"""DeepUPE dataset and datamodule.
"""
    ...
    def list_deepupe_files(self):
        ...
        with progress_bar() as pbar:
            ...
            for image_path in pbar.track(
                ...
                description=f"[bright_yellow]Listing DeepUPE {self.split} images"
            ):
                eimage_path       = image_path.replace("low", "expertc_high")
                custom_label_path = custom_label_path.replace("low", "annotations_custom")
...
class DeepUPEDataModule(DataModule):
    ...
    def __init__(
        self,
        dataset_dir: str = os.path.join(datasets_dir, "deepupe"),
        name       : str = "deepupe",
        *args, **kwargs
    ):
    ...
    def setup(self, phase: Optional[Phase] = None):
        ...
        """
        console.log(f"Setup [red]DeepUPE[/red] datasets.")
