...
"""Snow100K datasets and datamodules.
"""
...
data = {
    ...
    # Dataset's name.
    "snow_size": ["*"],
    # Snow size subset to use. One of ["snow100k_s", "snow100k_m", "snow100k_l"].
    # Can also be a list to include multiple snow sizes. When `all`, `*` or
    # `None`, all subsets
...
class Snow100K(ImageEnhancementDataset):
    ...
    Args:
        subset (Tasks, optional):
            ...
            all subsets will be included. Default: `*`.
        snow_size (Tasks, optional):
            Snow size subset to use. One of the values in `self.subsets`.
            Can also be a list to include multiple snow100k sizes. When `all`, `*`
            or `None`, all subsets will be included. Default: `*`.
    """
    
    snow_sizes = ["snow100k_s", "snow100k_m", "snow100k_l"]
    ...
    @property
    def snow_size(self) -> list[str]:
        return self._snow_size

    @snow_size.setter
    def snow_size(self, snow_size: Optional[Tasks]):
        snow_size = snow_size if isinstance(snow_size, str) else snow_size
        if snow_size is None or "all" in snow_size or "*" in snow_size:
            snow_size = self.snow_sizes
        self._snow_size = snow_size
    ...
    def list_files(self):
        ...
        if self.split == "train":
            ...
            raise ValueError(f"Snow100K dataset only supports `split`: "
    ...
    def list_train_files(self):
        ...
        with progress_bar() as pbar:
            eimage_pattern = os.path.join(
                self.root, "train", "gt", "*.jpg"
            )
            for eimage_path in pbar.track(
                glob.glob(eimage_pattern),
                description=f"[bright_yellow]Listing Snow100k train images"
            ):
                image_path        = eimage_path.replace("gt", "synthetic")
                custom_label_path = eimage_path.replace("gt", "annotations_custom")
    ...
    def list_test_files(self):
        ...
        with progress_bar() as pbar:
            eimage_pattern = os.path.join(
                self.root, "test", f"{self.snow_size}", "gt", "*.jpg"
            )
            for eimage_path in pbar.track(
                glob.glob(eimage_pattern),
                description=f"[bright_yellow]Listing Snow100k test images"
            ):
                image_path        = eimage_path.replace("gt", "synthetic")
                custom_label_path = eimage_path.replace("gt", "annotations_custom")
                custom_label_path = custom_label_path.replace(".jpg", ".json")
...
class Snow100KDataModule(DataModule):
    ...
    def __init__(
        self,
        dataset_dir: str = os.path.join(datasets_dir, "snow100k"),
        name       : str = "snow100k",
        *args, **kwargs
    ):
