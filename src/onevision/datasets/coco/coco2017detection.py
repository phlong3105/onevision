...
from onevision.data import CocoDetectionLabelHandler
...
# MARK: - COCO2017Detection

@DATASETS.register(name="coco2017detection")
class COCO2017Detection(ObjectDetectionDataset):
    ...
    def list_files(self):
        ...
        if self.split not in ["train", "val"]:
            raise ValueError(f"COCO2017 Detection dataset only supports "
        ...
        with progress_bar() as pbar:
            custom_label_dir = os.path.join(
                self.root, "2017", "annotations", "instances_custom"
            )
    ...
    def load_class_labels(self):
        """Load ClassLabels."""
        path = os.path.join(self.root, "2017", "80_class_labels.json")
        if not os.path.isfile(path):
            current_dir = os.path.dirname(os.path.abspath(__file__))
            path = os.path.join(current_dir, "coco2017_80_class_labels.json")
        self.class_labels = ClassLabels.create_from_file(label_path=path)
    

# MARK: - COCO2017DetectionDataModule

@DATAMODULES.register(name="coco2017detection")
class COCO2017DetectionDataModule(DataModule):
    ...
    def setup(self, phase: Optional[Phase] = None):
        ...
        if phase in [None, Phase.TRAINING]:
            self.train = COCO2017Detection(
                root=self.dataset_dir, split="train", **self.dataset_kwargs
            )
            self.val = COCO2017Detection(
...
if __name__ == "__main__":
    # NOTE: Get DataModule
    cfgs = data
    dm   = COCO2017DetectionDataModule(**cfgs)
    dm.setup()
