...
"""KODAS2019 LoL dataset and datamodule.
"""
...
class KODAS2019LoL(ImageEnhancementDataset):
    """KODAS2019 LoL dataset for low-light image enhancement task.
    ...
    def list_files(self):
        ...
        if self.split not in ["train", "val", "test"]:
            raise ValueError(f"KODAS2019 LoL dataset only supports `split`: "
                             f"`train`, `val`, or `test`. Get: {self.split}.")
        
        # NOTE: List image files
        with progress_bar() as pbar:
            image_pattern = os.path.join(
                self.root, "kodas2019_lol", self.split, "low", "*", "*.png"
            )
...
# MARK: - KODAS2019LoLDataModule

@DATAMODULES.register(name="kodas2019lol")
class KODAS2019LoLDataModule(DataModule):
    """KODAS2019 LoL DataModule."""
    ...
    def setup(self, phase: Optional[Phase] = None):
        ...
        """
        console.log(f"Setup [red]KODAS2019 LoL[/red] datasets.")
        
        # NOTE: Assign train/val datasets for use in dataloaders
        if phase in [None, Phase.TRAINING]:
            self.train = KODAS2019LoL(
                root=self.dataset_dir, split="train", **self.dataset_kwargs
            )
            self.val = KODAS2019LoL(
