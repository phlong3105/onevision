...
"""KODAS2020 Detection 2D dataset and datamodule.
"""
...
from onevision.cv import compute_box_area
from onevision.cv import draw_box
...
from onevision.data import ObjectAnnotation
from onevision.data import ObjectDetectionDataset
...
from onevision.file import is_txt_file
...
__all__ = [
    "KODAS2020Detection2D",
    "KODAS2020Detection2DDataModule"
]
...
"""KODAS Detection 2D label format:
...
# MARK: - KODAS2020Detection2D

@DATASETS.register(name="kodas2020det2d")
class KODAS2020Detection2D(ObjectDetectionDataset):
    """KODAS2020 Detection 2D Dataset consists of multiple sub-datasets
    ...
    def list_files(self):
        ...
        if self.split not in ["train", "val"]:
            raise ValueError(f"KODAS2020 Detection 2D dataset only supports "
                             f"`split`: `train` or `val`. Get: {self.split}.")
        
        # NOTE: List image files
        with progress_bar() as pbar:
            image_pattern = os.path.join(
                self.root, f"kodas2020_det2d", self.split, "images", "*.png"
            )
            self.image_paths = [
                p for p in
                pbar.track(
        ...
        with progress_bar() as pbar:
            custom_label_paths = [p.replace("images", "annotations_custom")
    ...
    def load_class_labels(self):
        """Load ClassLabels."""
        path = os.path.join(self.root, "kodas2020_det2d", "class_labels.json")
        if not os.path.isfile(path):
            current_dir = os.path.dirname(os.path.abspath(__file__))
            path = os.path.join(current_dir, "../../../../../onedetection/src/onedetection/datasets/kodas/kodas2020_class_labels.json")
        self.class_labels = ClassLabels.create_from_file(label_path=path)


# MARK: - KODAS2020Detection2DDataModule

@DATAMODULES.register(name="kodas2020det2d")
class KODAS2020Detection2DDataModule(DataModule):
    """KODAS2020 Detection 2D DataModule."""
    ...
    def setup(self, phase: Optional[Phase] = None):
        ...
        Args:
            phase (Phase, optional):
                ...
                Default: `None`.
        """
        console.log(f"Setup [red]KODAS2020 Detection 2D[/red] datasets.")
