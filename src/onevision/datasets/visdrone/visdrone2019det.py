...
from onevision.cv import box_xywh_to_cxcywh_norm
from onevision.cv import box_xywh_to_xyxy
...
__all__ = [
    "VisDrone2019Detection",
    "VisDrone2019DetectionDataModule"
]
...
@DATASETS.register(name="visdrone2019det")
class VisDrone2019Detection(ObjectDetectionDataset):
    """FVisDrone2019 dataset consists of multiple sub-datasets captured by
        ...
        *args, **kwargs
        ...
        if not os.path.isfile(class_labels_path):
            ...
            class_labels_path = os.path.join(
                curr_dir, f"visdrone2019_class_labels.json"
            )
    ...
    def list_files(self):
        ...
        with progress_bar() as pbar:
            image_pattern = os.path.join(
                self.root, f"det2019", self.split, "images", "*.jpg"
            )
            self.image_paths = [
    ...
    def load_class_labels(self):
        """Load ClassLabels."""
        path = os.path.join(self.root, "det_2019", "class_labels.json")
        if not os.path.isfile(path):
            current_dir = os.path.dirname(os.path.abspath(__file__))
            path 	    = os.path.join(current_dir,
                                       "visdrone2019_class_labels.json"
                                       )
...
@DATAMODULES.register(name="visdrone2019det")
class VisDrone2019DetectionDataModule(DataModule):
    ...
    def setup(self, phase: Optional[Phase] = None):
        ...
        Args:
            phase (Phase, optional):
                ...
                Default: `None`.
        """
        console.log(f"Setup [red]VisDrone2019 Detection[/red] datasets.")
    ...
    def load_class_labels(self):
        """Load ClassLabels."""
        current_dir = os.path.dirname(os.path.abspath(__file__))
        path 	    = os.path.join(current_dir, "visdrone2019_class_labels.json")
        self.class_labels = ClassLabels.create_from_file(label_path=path)
