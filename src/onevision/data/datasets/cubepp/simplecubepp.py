...
"""SimpleCube++ dataset and datamodule.

In addition to the main Cube++ dataset, small and simpler version of it is
prepared. Simple dataset contains 4x downscaled images that have less than 1°
difference between left and right ground truth illumination estimation.
It includes only 2234 images with single source illumination; consequently,
there is only one ground truth per image. The simple dataset weights around 2GB.

Small dataset contains PNG and JPG files, gt.csv with ground truth data and
properties.csv with manual annotation data:
...
class SimpleCubePP(ImageEnhancementDataset):
    ...
    def list_simplecubepp_files(self):
        """List all SimpleCube++ image data."""
        with progress_bar() as pbar:
            image_pattern = os.path.join(
                self.root, "simplecube++", self.split, "png", "*.png"
            )
            for image_path in pbar.track(
                glob.glob(image_pattern),
                description=f"[bright_yellow]Listing SimpleCube++ {self.split} images"
            ):
...
class SimpleCubePPDataModule(DataModule):
    ...
    def setup(self, phase: Optional[ModelState] = None):
        ...
        """
        console.log(f"Setup [red]SimpleCube++[/red] datasets.")
...
if __name__ == "__main__":
    ...
    dm   = SimpleCubePPDataModule(**cfgs)
    dm.setup()
