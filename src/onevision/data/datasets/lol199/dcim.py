...
class DCIM(UnsupervisedImageDataset):
    """DCIM dataset consists 64 low-light images.
    """
...
class DCIMDataModule(DataModule):
    ...
    def setup(self, phase: Optional[ModelState] = None):
        ...
        if phase in [None, ModelState.TRAINING]:
            full_dataset = DCIM(root=self.dataset_dir, **self.dataset_kwargs)
        ...
        if phase in [None, Phase.TESTING]:
            self.test         = DCIM(root=self.dataset_dir, **self.dataset_kwargs)
