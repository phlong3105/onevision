...
class LIME(UnsupervisedImageDataset):
    """LIME dataset consists 10 low-light images.
    """
...
class LIMEDataModule(DataModule):
    ...
    def setup(self, phase: Optional[Phase] = None):
        ...
        if phase in [None, Phase.TRAINING]:
            full_dataset = LIME(root=self.dataset_dir, **self.dataset_kwargs)
        ...
        if phase in [None, Phase.TESTING]:
            self.test         = LIME(root=self.dataset_dir, **self.dataset_kwargs)
