...
from onevision.data.dataset import UnsupervisedImageDataset
...
@DATASETS.register(name="lol199")
class LoL199(UnsupervisedImageDataset):
    """LoL199 dataset consists 199 low-light images.
    """
    ...
    def list_files(self):
        ...
        # NOTE: List all files
        self.list_dcim_files()
        self.list_lime_files()
        self.list_mef_files()
        self.list_npe_files()
        self.list_vip_files()
        self.list_vv_files()
    ...
    def list_dcim_files(self):
        """List all DCIM image data."""
        with progress_bar() as pbar:
            image_pattern = os.path.join(self.root, "dcim", "low", "*.jpg")
            for image_path in pbar.track(
                glob.glob(image_pattern),
                description=f"[bright_yellow]Listing DCIM {self.split} images"
            ):
    ...
    def list_lime_files(self):
        """List all LIME image data."""
        with progress_bar() as pbar:
            image_pattern = os.path.join(self.root, "lime", "low", "*.bmp")
            for image_path in pbar.track(
                glob.glob(image_pattern),
                description=f"[bright_yellow]Listing LIME {self.split} images"
            ):
                custom_label_path = image_path.replace("low", "annotations_custom")
    ...
    def list_npe_files(self):
        """List all NPE image data."""
        with progress_bar() as pbar:
            image_pattern = os.path.join(self.root, "npe", "low", "*.*")
            for image_path in pbar.track(
                glob.glob(image_pattern),
                description=f"[bright_yellow]Listing NPE {self.split} images"
            ):
                custom_label_path = image_path.replace("low", "annotations_custom")
    ...
    def list_vip_files(self):
        """List all VIP image data."""
        with progress_bar() as pbar:
            image_pattern = os.path.join(self.root, "vip", "low", "*.png")
            for image_path in pbar.track(
                glob.glob(image_pattern),
                description=f"[bright_yellow]Listing VIP {self.split} images"
            ):
                custom_label_path = image_path.replace("low", "annotations_custom")
                custom_label_path = custom_label_path.split(".")[0]
                custom_label_path = f"{custom_label_path}.json"
    ...
    def list_vv_files(self):
        """List all VIP image data."""
        with progress_bar() as pbar:
            image_pattern = os.path.join(self.root, "vv", "low", "*.jpg")
            for image_path in pbar.track(
                glob.glob(image_pattern),
                description=f"[bright_yellow]Listing VV {self.split} images"
            ):
                custom_label_path = image_path.replace("low", "annotations_custom")
                custom_label_path = custom_label_path.split(".")[0]
                custom_label_path = f"{custom_label_path}.json"
...
# MARK: - LoL199DataModule

@DATAMODULES.register(name="lol199")
class LoL199DataModule(DataModule):
    """LoL199 DataModule."""
    ...
    def setup(self, phase: Optional[Phase] = None):
        ...
        """
        console.log(f"Setup [red]LoL199[/red] datasets.")
        
        # NOTE: Assign train/val datasets for use in dataloaders
        if phase in [None, Phase.TRAINING]:
            full_dataset = LoL199(root=self.dataset_dir, **self.dataset_kwargs)
        ...
        if phase in [None, Phase.TESTING]:
            self.test         = LoL199(root=self.dataset_dir, **self.dataset_kwargs)
