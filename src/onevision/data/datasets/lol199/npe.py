...
class NPE(UnsupervisedImageDataset):
    """NPE dataset consists 84 low-light images.
    """
...
class NPEDataModule(DataModule):
    ...
    def setup(self, phase: Optional[Phase] = None):
        ...
        if phase in [None, Phase.TRAINING]:
            full_dataset = NPE(root=self.dataset_dir, **self.dataset_kwargs)
        ...
        if phase in [None, Phase.TESTING]:
            self.test         = NPE(root=self.dataset_dir, **self.dataset_kwargs)
