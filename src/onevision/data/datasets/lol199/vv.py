...
class VV(UnsupervisedImageDataset):
    """VV dataset consists 25 low-light images.
    """
...
class VVDataModule(DataModule):
    ...
    def setup(self, phase: Optional[Phase] = None):
        ...
        if phase in [None, Phase.TRAINING]:
            full_dataset = VV(root=self.dataset_dir, **self.dataset_kwargs)
        ...
        if phase in [None, Phase.TESTING]:
            self.test         = VV(root=self.dataset_dir, **self.dataset_kwargs)
