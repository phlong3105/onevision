    ...
    def list_sid_files(self):
        ...
        with progress_bar() as pbar:
            ...
            for eimage_path in pbar.track(
                glob.glob(fuji_eimage_pattern),
                description=f"[bright_yellow]Listing SID {self.split} images"
            ):
                image_prefix  = eimage_path.split("_")[0].replace("high", "low")
                ...
                for image_path in glob.glob(image_pattern):
                    ...
                    custom_label_path = custom_label_path.replace(".raf", ".json")
            ...
            for eimage_path in pbar.track(
                glob.glob(sony_eimage_pattern),
                description=f"[bright_yellow]Listing {self.split} SID images"
            ):
                image_prefix  = eimage_path.split("_")[0].replace("high", "low")
                image_pattern = f"{image_prefix}*.arw"
                for image_path in glob.glob(image_pattern):
                    custom_label_path = image_path.replace("low", "annotations_custom")
                    custom_label_path = custom_label_path.replace(".arw", ".json")
                    self.image_paths.append(image_path)
                    self.eimage_paths.append(eimage_path)
                    # self.label_paths.append(label_path)
                    self.custom_label_paths.append(custom_label_path)
...
class SIDDataModule(DataModule):
    ...
    def __init__(
        self,
        dataset_dir: str = os.path.join(datasets_dir, "sid"),
        name       : str = "sid",
        *args, **kwargs
    ):
