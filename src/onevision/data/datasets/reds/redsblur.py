...
class REDSBlur(ImageEnhancementDataset):
    ...
    Args:
        subset (Tasks, optional):
            ...
            `None`, ONLY `blurry` and `blurry_compressed` subsets will be
            included. Default: `blurry`.
    """
    
    subsets = ["blurry", "blurry_compressed", "blurry_low_resolution"]
    ...
    def list_files(self):
        ...
        if self.split not in ["train", "val", "test"]:
            raise ValueError(f"REDS Blur dataset only supports `split`: "
                             f"`train`, `val`, or `test`. Get: {self.split}.")
        
        # NOTE: List all files
        if "blurry" in self.subset:
            self.list_blurry_files()
        if "blurry_compressed" in self.subset:
            self.list_blurry_compressed_files()
    ...
    def list_blurry_files(self):
        ...
        with progress_bar() as pbar:
            ...
            for image_path in pbar.track(
                ...
                description=f"[bright_yellow]Listing blurry {self.split} images"
            ):
    ...
    def list_blurry_compressed_files(self):
        """List all blurry & compressed files."""
        with progress_bar() as pbar:
            image_pattern = os.path.join(
                self.root, self.split, "blur_comp", "*", "*.png"
            )
            for image_path in pbar.track(
                glob.glob(image_pattern),
                description=f"[bright_yellow]Listing blurry & compressed {self.split} images"
            ):
                eimage_path       = image_path.replace("blur_comp", "sharp")
                custom_label_path = image_path.replace("blur_comp", "annotations_custom")
                custom_label_path = custom_label_path.replace(".png", ".json")
                self.image_paths.append(image_path)
                self.eimage_paths.append(eimage_path)
                # self.label_paths.append(label_path)
    ...
    def setup(self, phase: Optional[ModelState] = None):
        ...
        Args:
            phase (ModelState, optional):
                ...
                Default: `None`.
        """
        console.log(f"Setup [red]REDS Blur[/red] datasets.")
...
if __name__ == "__main__":
    ...
    cfgs = {
        ...
        # list to include multiple subsets. When `all`, `*`, or `None`, ONLY
        # `blurry` and `blurry_compressed` subsets will be included.
        # Default: `blurry`.
        "shape": [512, 512, 3],
