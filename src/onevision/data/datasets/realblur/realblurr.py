...
"""RealBlur-R dataset and datamodule.
"""
...
class RealBlurR(ImageEnhancementDataset):
    ...
    def list_files(self):
        ...
        if self.split == "train":
            ...
            raise ValueError(f"RealBlurR dataset only supports `split`: "
    ...
    def list_train_files(self):
        ...
        with progress_bar() as pbar:
            list_file = os.path.join(self.root, "realblur_r_train.txt")
            lines     = open(list_file, "r").readlines()
            for line in pbar.track(
                lines, description=f"[bright_yellow]Listing {self.split} images"
            ):
                eimage_path, image_path = line.rstrip().split(" ")
                image_path        = os.path.join(self.root, image_path)
                eimage_path       = os.path.join(self.root, eimage_path)
    ...
    def list_test_files(self):
        """List testing files."""
        with progress_bar() as pbar:
            list_file = os.path.join(self.root, "realblur_r_test.txt")
            lines     = open(list_file, "r").readlines()
            for line in pbar.track(
                lines, description=f"[bright_yellow]Listing {self.split} images"
            ):
...
# MARK: - RealBlurrDataModule

@DATAMODULES.register(name="realblurr")
class RealBlurRDataModule(DataModule):
    """RealBlurR DataModule."""
    
    # MARK: Magic Functions
    
    def __init__(
        self,
        dataset_dir: str = os.path.join(datasets_dir, "realblur"),
        name       : str = "realblurr",
        *args, **kwargs
    ):
