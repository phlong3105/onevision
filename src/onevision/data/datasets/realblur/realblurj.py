...
class RealBlurJ(ImageEnhancementDataset):
    ...
    def list_files(self):
        ...
        if self.split == "train":
            ...
            raise ValueError(f"RealBlurJ dataset only supports `split`: "
    ...
    def list_train_files(self):
        ...
        with progress_bar() as pbar:
            list_file = os.path.join(self.root, "realblur_j_train.txt")
    ...
    def list_test_files(self):
        """List testing files."""
        with progress_bar() as pbar:
            list_file = os.path.join(self.root, "realblur_j_test.txt")
            lines     = open(list_file, "r").readlines()
            for line in pbar.track(
                lines, description=f"[bright_yellow]Listing {self.split} images"
            ):
