...
"""RDN trained on Div2K X2 dataset.
"""
...
callbacks = [
	{
		...
		# Default: `True`.
		"monitor": "checkpoint/psnry/val_epoch",  # "loss_epoch",
...
trainer = {
	...
	# Gradient clipping algorithm to use. Pass `gradient_clip_algorithm="value"`
	# to clip by value, and `gradient_clip_algorithm="norm"` to clip by norm.
	# By default it will be set to `norm`. Default: `None`.
	"ipus": None,
...
model = {
	...
	"metrics": {
		"train": [{"name": "psnry"}],
		"val"  : [{"name": "psnry"}, {"name": "ssim"}],
		"test ": [{"name": "psnry"}, {"name": "ssim"}],
	},
	# Metrics' configs.
    "optimizers": [
		{
