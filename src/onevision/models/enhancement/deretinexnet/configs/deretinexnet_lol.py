...
"""DeRetinexNet trained on LoL dataset.
"""
...
inference = {
    ...
    "version": None,
    # Experiment version. If version is not specified the logger inspects
	# the save directory for existing
    # versions, then automatically assigns the next available version. If it
	# is a string then it is used as the run-specific subdirectory name,
...
data = {
    ...
    # Datasets" name.
    "subset": "lol",
    # Sub-dataset name. One of: ["deepupe", "dped", "gladnet", "lol",
    # "sice", "sid"]. Can also be a list to include multiple subsets.
    # When `all`, `*`, or `None`, all subsets will be included. Default: `lol`.
    "exposure": "1",
...
model = {
	...
	# Default: `-1`.
	"loss": None,
	# Loss config. Define in the code.
    "metrics": {
		"train": [{"name": "psnr"}],
    ...
    "optimizers": [
		{
			"optimizer": {"name": "adam", "lr": 0.00001},
			"lr_scheduler": None,
			"frequency": None,
		},
    ],
